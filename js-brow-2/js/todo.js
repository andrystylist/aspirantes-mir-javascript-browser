(function () {
  ...
  function handleSubmit(event) {
    ...
    const todoText = todoInput.value.trim(); // obtiene el texto del input
    
    if (todoText.length === 0) {
      return; // si el texto esta vacío no hace nada
    }
    
    const nuevaTarea = createTasks(todoText); // nueva tarea y devuelve el objeto
    addTaskToList(nuevaTarea);  // agrega la tarea al array de tareas y al Dom
    todoInput.value = ''; //lima el input
